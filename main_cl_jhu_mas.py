...
# ================= MAS工具类 ===================
class MAS:
    def __init__(self, model, dataloader, device, sample_num=200):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for n, p in self.model.named_parameters():
                if p.grad is not None and p.requires_grad:
                    importance[n] += torch.abs(p.grad.detach())
            count += 1
            
        # 归一化重要性
        for n in importance:
            importance[n] /= max(count, 1)
...
# =================== 主训练流程 =====================
def mas_train_and_evaluate_trial(cfg, args, device, master_seed):
    ...
    for task_id in range(len(tasks_train)):
        ...
        for epoch in range(args.epochs_per_task):
            ...
            for batch in tqdm(task_loader, desc=f"Task {task_id} Epoch {epoch+1}"):
                try:
                    ...
                    # MAS penalty
                    if len(mas_list) > 0:
                        mas_penalty = sum([mas.penalty(model) for mas in mas_list])
                        loss_total = loss_total + mas_lambda * mas_penalty
            ...
            if val_count > 0:
                ...
                if perf_current['mae'] < best_perf_each_task[task_id]['mae']:
                    ...
                    }, f'checkpoints/mas/task_{task_id}_best.pth')
        ...
        # 训练完当前任务，保存MAS
        print(f"计算MAS重要性并保存...")
        mas = MAS(model, task_loader, device)
        mas_list.append(mas)
    ...
    }, 'checkpoints/mas/final_model.pth')
    print("\n保存最终模型到 checkpoints/mas/final_model.pth")
...
def main():
    ...
    parser.add_argument("--clearml_task", type=str, default="JHU_ContinualLearning_MAS",
    ...
    master_seed = 1000
    mas_train_and_evaluate_trial(cfg, args, device, master_seed)
